/**
 * SecurityEventRepositoryDdb - sandbox implementation for local Consentrail.
 */
import type { SecurityEventRepository } from "@consentrail/services/security-events";
import { id, listOf, one, store } from "../_shared/product-sandbox.js";

export class SecurityEventRepositoryDdb implements SecurityEventRepository {
  constructor(private readonly dynamoClient: any) {}

  async listSecurityEvents(input: any) {
    return listOf(store.securityEvents, input?.correlationId) as any;
  }
  async recordSecurityEvent(input: any) {
  ...
  async getSecurityEvent(input: any) {
    ...
    return one(store.securityEvents.get(id), input?.correlationId) as any;
  }
}
