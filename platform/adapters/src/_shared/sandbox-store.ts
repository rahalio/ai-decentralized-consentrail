...
export function getOrCreateTenantProfile(tenantId: string): SandboxTenantProfile {
  let profile = tenantProfilesById.get(tenantId);
  if (!profile) {
    profile = {
      tenantId,
      displayNameEn: 'Consentrail Demo Consortium',
      displayNameAr: 'كونسورتيوم كونسنتريل التجريبي',
    };
...
export function generateApiKeySecret(prefix = 'consentrail_demo'): string {
