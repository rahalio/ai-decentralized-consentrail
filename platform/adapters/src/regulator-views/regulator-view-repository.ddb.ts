/**
 * RegulatorViewRepositoryDdb - sandbox implementation for local Consentrail.
 */
import type { RegulatorViewRepository } from "@consentrail/services/regulator-views";
import { id, listOf, one, store } from "../_shared/product-sandbox.js";

export class RegulatorViewRepositoryDdb implements RegulatorViewRepository {
  constructor(private readonly dynamoClient: any) {}

  async listRegulatorViews(input: any) {
    return listOf(store.regulatorViews, input?.correlationId) as any;
  }
  async grantRegulatorView(input: any) {
  ...
  async getRegulatorView(input: any) {
    ...
    return one(store.regulatorViews.get(id), input?.correlationId) as any;
  }
}
