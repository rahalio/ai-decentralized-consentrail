...
1. This product already uses `@consentrail/*`. Restore `.codegen/` from the scaffold if missing (never commit it).
2. Keep `common/` + `identity`; add product domains under OpenAPI.
3. Extend `_shared` ID prefix maps and event catalogs — do not fork middleware.
